...
`1378b05dcb8a3442135b318911fb62b55ccdc92a` passes 631/631 tests across 55
...
`de66cb8c-f9b6-464e-8741-8aed362d7955`; exact release
...
The candidate Wrangler dry-run reports 248 official assets, 1136.77 KiB raw /
209.71 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
browser evidence below is kept distinct from those local gates.

The deployed increment connects Simple Alarms, Treatment Notify and Timeago to
the core notification processor through one persisted task. Simple Alarms
...
mutations now evaluate all three producers in upstream order on the leading
edge. The task retains the earliest activation, heartbeat or expiry deadline.
...
Both branches remain dormant under the public upstream-default settings.
External delivery is not connected. The prior
...
exclusions. Version 64 passed remote API/EIO4 and real-browser gates.
...
default test-file status is `unresolved`. At this audit point, 39 files are
`unresolved`, two real-CGM bridge files are `excluded-fixed-scope`, 69 files
...
Runtime candidate
`1378b05dcb8a3442135b318911fb62b55ccdc92a` passes 631/631 tests in 55
...
files. It connects schema-v14 background work to automatic Simple Alarm,
Treatment Notify and Timeago evaluation while retaining the complete named
...
Cloudflare version `de66cb8c-f9b6-464e-8741-8aed362d7955` (ordinal 64) is
100% active; its deployment was created at `2026-07-20T09:27:03.757Z` and
reported a 25 ms startup. Wrangler 4.112.0 does not expose a separate
deployment UUID in the current list output, so none is invented. Wrangler
processed 248 unchanged official asset entries; the final dry run reported
1136.77 KiB raw / 209.71 KiB gzip, with only `ENTRY_STORE` and `ASSETS`.
...
API3 Entries returned 401. Isolated tenant `public-smoke-1784539642541` reported
...
- `/alarm` connects independently. Native access tokens take priority; web
  ...
  feeds automatic Simple Alarm, enabled Treatment Notify and opt-in Timeago
  results through this same outlet; other automatic plugin evaluations remain
  missing;
...
fresh tenant `public-smoke-1784539642541` and observed 237,568 SQLite bytes. It
sent no API secret, inspected no dashboard credential state and performed no
protected mutation.

The deployed version adds automatic Simple Alarms, Treatment Notify and Timeago
evaluation through the core notification processor while retaining Basal
Profile, OpenAPS/Pump, IOB/COB and
treatment-to-curve,
...
A real Chromium session reloaded Cloudflare version 64, rendered the official homepage,
...
Fresh main-workflow listeners recorded no warnings and no JavaScript errors.
The official Settings form opened and
`Save` was clicked with unchanged defaults; it stayed at zero visible forms for
3.5 seconds, directly rechecking the reported pop-back symptom. No authenticated
server mutation was attempted, and the isolated browser tab was closed. This is
same-version evidence alongside version 64's remote API and Engine.IO smoke.
...
6. Add bounded tick/prune/plugin producers to the deployed persisted alarm
   table; Simple Alarms, enabled Treatment Notify and opt-in Timeago already
   share the first automatic task.
