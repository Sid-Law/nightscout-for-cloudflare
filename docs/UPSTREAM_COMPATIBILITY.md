...
`58b33fc2b068a42a033be29ea8d2ca34fef0081a` passes 687/687 tests across 62
...
`83c08370-341c-4c4c-b7ea-4378e499eb24`; exact release
...
The candidate Wrangler dry-run reports 248 official assets, 1175.66 KiB raw /
217.19 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
browser evidence below is kept distinct from those local gates.

The deployed increment connects AR2, Simple Alarms, Pump, OpenAPS, Loop,
Treatment Notify and Timeago to the core notification processor through one
persisted task. AR2 preserves the official coefficients, six-point projection,
...
mutations now evaluate all seven producers in official server order on the
...
Runtime candidate
`58b33fc2b068a42a033be29ea8d2ca34fef0081a` passes 687/687 tests in 62
...
files. It connects schema-v14 background work to automatic AR2, Simple Alarm, Pump,
...
Cloudflare version `83c08370-341c-4c4c-b7ea-4378e499eb24` (ordinal 73) is 100%
active; deployment metadata activated at `2026-07-21T23:44:59.295697Z`, and it
reported a 25 ms startup. Wrangler
processed 248 unchanged official asset entries; the final dry run reported
1175.66 KiB raw / 217.19 KiB gzip, with only `ENTRY_STORE` and `ASSETS`.
...
API3 Entries returned 401. Isolated tenant `public-smoke-1784677520758` reported
...
fresh tenant `public-smoke-1784677520758` and observed 262,144 SQLite bytes. It
...
A real browser session loaded Cloudflare version 73, rendered the official
homepage, chart region and exactly 26 AR2 forecast dots. The simulated stream
was stale, so the headline correctly showed `---`; the only console rejections
were browser autoplay-policy errors before user interaction, not application or
API failures. In the broader version-72 pass, the live database-size pill showed
...
6. Add bounded tick/prune/plugin producers to the deployed persisted alarm
   table; AR2, Simple Alarms, Pump, OpenAPS, Loop, enabled Treatment Notify and
