...
The current local suite has 26 Workers-runtime tests. The locked
...
| Collections and indexes | `lib/storage/mongo-storage.js`, server storage modules | **Partial.** SQLite tables cover SGV entries plus generic documents. Internal schema v4 now adds indexed treatments metadata, collection clocks and atomic document-change snapshots. Identifier/fallback indexes are intentionally non-unique, as upstream does not declare uniqueness. | Extend the v4 contract and migration/failure fixtures from treatments to every enabled collection. |
| ObjectId, UUID and dedupe | `lib/server/query.js`, `lib/server/treatments.js`, `lib/api3/storage/mongoCollection/utils.js` | **Partial treatments slice.** Generated IDs are random 24-hex strings. Treatments repository lookup/upsert now uses identifier, `_id`, then legacy `created_at + eventType`, preserves the server ID on retransmit, and normalizes non-ObjectId `_id` to `identifier`. IDs are not BSON ObjectIds. | Add full immutable-field, duplicate-row, batch/partial-failure and every-collection fixtures. |
| Mongo query behavior | `lib/server/query.js`, `lib/api3/storage/mongoCollection/**` | **Partial.** Existing v1 generic filtering remains limited. The new treatments repository pushes supported filters, ordering, offset and limit into SQLite and supports field projection; it does not pre-limit 5,000 JSON documents. | Complete operator/type/array/regex/logical-query parity and wire the repository into the future API v3 request adapter. |
...
| API v3 generic/lastModified/history | `lib/api3/generic/**`, `specific/lastModified.js` | **Partial storage foundation, routes missing.** Treatments now have schema-v4 metadata, monotonic last-modified, create/upsert/replace/patch, tombstones, permanent deletion and ascending projected history behind DO RPCs. No `/api/v3/treatments` CRUD/history or `/api/v3/lastModified` route is wired. | Add the authenticated HTTP adapter, exact validation/envelopes/conditional headers/renderers, then port all upstream `api3.*.test.js` workflows. |
