...
Deployed runtime candidate `7298f45` passes 749/749 tests across 68
...
`0dd4cdd5-09d2-428e-b12e-d6d66e6905e3`; exact release
...
The candidate Wrangler dry-run reports 250 Static Assets entries, 1256.42 KiB raw /
230.69 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
browser evidence below is kept distinct from those local gates.

The deployed increment connects AR2, Simple Alarms, Error Codes, Pump, OpenAPS, Loop, BWP,
...
mutations now evaluate all thirteen producers in official server order on the
...
| Server plugins and calculations | `lib/plugins/index.js`, `lib/sandbox.js`, `lib/data/{dataloader,treatmenttocurve}.js`, `lib/profilefunctions.js` | **Static registry, complete Sandbox/dataloader/dbsize/age/timeago/AR2/Basal/Treatment-Notify/Simple-Alarms/Error-Codes/OpenAPS/Pump/IOB/COB/BWP/treatment-curve surfaces and thirteen automatic producers; broader background execution partial.** Pure ports of official `bgnow`, `direction`, `rawbg`, `upbat`, `ar2`, `basal`, `treatmentnotify`, `simplealarms`, `errorcodes`, `loop`, `openaps`, `pump`, `iob`, `cob`, `bwp`, `dbsize`, age/timeago, treatment-to-curve, shared runtime helpers and Profile calculations support v2 without rewriting formulas. BWP additionally maps its missing-data, unit, target, Temp Basal, notification and snooze paths and exposes Pebble fields. AR2 maps all ten named cases; Simple Alarms maps all five; Error Codes maps all eight unchanged upstream cases plus ten pure and three real-DO cases; Basal maps two and Treatment Notify six; OpenAPS/Pump map all 16 and IOB/COB/treatment-to-curve all 24 named cases plus DO/HTTP integration. The request-local Sandbox and static registry retain their complete contracts. Eleven complete client files run 42/42 unchanged, and twenty-one server/data-plugin files run 143/143 unchanged, against the locked upstream. AR2, Simple Alarms, Error Codes, Pump, OpenAPS, Loop, BWP, CAGE, SAGE, IAGE, enabled Treatment Notify, opt-in Timeago and opt-in DBSize alerts share the persisted scheduler; unported algorithms are not fabricated. | Adapt the remaining plugin files through the Sandbox and deterministic tenant platform context, then add their automatic producers without inventing algorithms. |
| Notifications/admin state | `lib/notifications.js`, `lib/api/notifications-api.js`, `lib/adminnotifies.js`, `lib/server/pushnotify.js`, `lib/plugins/{maker,pushover}.js` | **Core notification processor, ACK/outlet persistence, thirteen automatic producers, Admin notices and provider protocol state adapted.** Schema v13 preserves last-emission state, v14 evaluates thirteen producers including default Error Codes and opt-in BWP, v15 persists Admin notices, and v18 persists Pushnotify dedupe/receipts plus Maker All Clear state. All 15 locked Maker/Pushover/Pushnotify cases are represented, and the v1/v2 receipt callback consumes only a stored unexpired receipt. Activity/summary persistence, remaining plugin-bus integration and live external send/cancel remain missing. | Add the remaining alarm producers and activity/summary persistence; connect external delivery only after explicit destination authorization and a persisted outbox. |
...
Runtime candidate `7298f45` passes 749/749 tests in 68
...
Error Codes, Pump, OpenAPS, Loop, BWP, CAGE, SAGE, IAGE, Treatment Notify, Timeago and opt-in
...
Cloudflare version `0dd4cdd5-09d2-428e-b12e-d6d66e6905e3` (ordinal 88) is
current and reported a 28 ms startup. Wrangler processed 250 Static Assets entries; the
final dry run reported 1256.42 KiB raw / 230.69 KiB gzip, with only
`ENTRY_STORE` and `ASSETS`.
This deployment had no explicit version annotation; none is invented.

The reusable credential-free remote smoke passed 121 assertions for health,
...
default-enabled Error Codes status, missing-token
API3 Entries returned 401. Isolated tenant `public-smoke-1784700236170` reported
