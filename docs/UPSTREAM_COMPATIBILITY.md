...
Deployed runtime candidate `ab101f5` passes 793/793 tests across 72
...
`37ceecb8-4f67-40c0-8af2-cfcd06ff8910`; exact release
...
The candidate Wrangler dry-run reports 250 Static Assets entries, 1315.83 KiB raw /
242.12 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
...
Runtime candidate `ab101f5` passes 793/793 tests in 72
...
Cloudflare version `37ceecb8-4f67-40c0-8af2-cfcd06ff8910` is current as
project release and Cloudflare ordinal 99 and reported a 27 ms startup. It
processed 250 Static Assets entries; the final dry run reported 1315.83 KiB raw
/ 242.12 KiB gzip, with only
...
queue. The 177-assertion post-deploy smoke passed on
`public-smoke-1784725389106` with both Engine.IO generations and the existing
direct/upgrade WSS gates. The unchanged homepage rendered two SVGs without a
dialog and advanced from `125 +4 ↗` to `128 +4 ↗` while remaining at `/`.
