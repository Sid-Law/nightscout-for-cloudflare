...
`94b64e3e417806ddfa5243834ee8ce7eb9bc2f41` passes 577/577 tests across 50
Workers-runtime files plus 21/21 audit tests, one complete unchanged
upstream-client test and 53/53 unchanged tests across nine locked upstream
...
`1ed7fda2-c6bc-4137-9f53-25fcc16d8f40`; exact release
...
The candidate Wrangler dry-run reports 248 official assets, 1070.53 KiB raw /
196.22 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
browser evidence below is kept distinct from those local gates.

The deployed increment adds request-local IOB, COB and treatment-to-curve ports
to the static plugin registry and v2 data path. IOB/COB retain the official
...
ordinary-Treatment Free-plan cap under the existing JSON budget. The prior
dataloader/database-size, age/timeago, Sandbox, Profile, units and times ports
remain request-local. Missing algorithms are not fabricated, and no dosing
...
The manifest is one `pass`, 67 `adapted`, 41 `unresolved` and two fixed-scope
exclusions. Version 59 passed remote API/EIO4 and real-browser gates.
...
default test-file status is `unresolved`. At this audit point, 41 files are
`unresolved`, two real-CGM bridge files are `excluded-fixed-scope`, 67 files
are `adapted`, and `pluginbase.test.js` is the one unchanged `pass`.
...
| API v2 properties and ddata | `lib/api2/index.js`, `lib/data/{endpoints,dataloader,treatmenttocurve}.js`, `lib/api2/properties.js`, `lib/{times,units,levels}.js`, `lib/plugins/{bgnow,direction,rawbg,upbat,loop,iob,cob,dbsize,cannulaage,sensorage,insulinage,timeago}.js` | **Eighteen named files adapted; wider plugin-property surface partial.** `/ddata/at` selects live versus explicit frames with a bounded two-day SGV window, publishes the real tenant SQLite file total and applies official Treatment-to-curve marker fields. The prior complete data/property files plus all 14 IOB, nine COB and one treatment-to-curve cases are represented. `/properties`, wildcard/comma selection and truthy `pretty` are deployed; default uploader/database-size and opt-in Loop/IOB/COB/CAGE/SAGE/IAGE execute through the registry. The projection includes 64 SGVs, latest calibration, recent DeviceStatus/stats/current Profile, one latest 62-day age event per type, one-year zero-duration Profile Switch and the newest 1,000 ordinary Treatments from the upstream 2.5-day window. Timeago is correctly not fabricated as a property. IOB/COB preserve official formulas and do not recommend a dose. The rolling-deploy adapter falls back only when an old live DO lacks the property-context RPC. | Add every remaining property produced by the official server plugin registry, then extend endpoint/error/retro and multi-device differential fixtures beyond the eighteen complete named files. Keep the 1,000-Treatment Free-plan cap explicit. |
...
| Server plugins and calculations | `lib/plugins/index.js`, `lib/sandbox.js`, `lib/data/{dataloader,treatmenttocurve}.js`, `lib/profilefunctions.js` | **Static registry, complete Sandbox/dataloader/dbsize/age/timeago/IOB/COB/treatment-curve surfaces and expanded request-scoped property, Loop and Profile foundation; background execution missing.** Pure ports of official `bgnow`, `direction`, `rawbg`, `upbat`, `loop`, `iob`, `cob`, `dbsize`, age/timeago, treatment-to-curve, shared runtime helpers and Profile calculations support v2 without rewriting formulas. IOB/COB/treatment-to-curve map all 24 named upstream cases plus DO/HTTP integration. The request-local Sandbox and static registry retain their complete contracts. The client pluginbase file and nine server/data-plugin files run unchanged against the locked upstream for 53/53 tests. Loop/dbsize/age/timeago timer/persistence outlets and unported algorithms are not connected or fabricated. | Adapt the remaining plugin files through the Sandbox and deterministic tenant platform context, then add notification/background execution without inventing algorithms. |
...
| Official page workflows | `views/**`, browser client/admin/report modules | **Partial.** Version 59's credential-free Chromium pass rendered the homepage/chart and live `0%` database-size pill, then loaded Admin Tools, Food Editor, Profile Editor and `clock-color`. Food reached `Database loaded`; Profile reached `Values loaded.` with its stored simulated profile and `Asia/Shanghai` timezone; the empty-data clock rendered `-?-`. Fresh per-page listeners recorded zero JavaScript errors, failed requests and HTTP error responses. Admin/Food/Profile emitted only the locked bundle's nonfatal missing-`#chartContainer` warning on those non-chart pages. The official Settings form closed and stayed at zero visible forms for three seconds. An earlier increment provided authenticated Profile Save/close evidence, but no protected Save was attempted in version 59. The public tenant has no Entries, so `---` is expected. Mutations/report generation and pushed live updates are not complete. | Re-run authenticated Profile Save/Food mutation only when a credential is explicitly available, then add profile delete, admin mutations, report generation and pushed live updates with console/network assertions. |
| Upstream test tracking | `tests/**`, `upstream/contract-manifest.json`, `scripts/audit-upstream-contracts.mjs` | **Inventory complete; one direct pass and 67 adapted files.** All 111 files are tracked with strict status/reason and heuristic route candidates: `pluginbase.test.js` runs unchanged, while all 16 API3 files, the named storage/concurrency/notification/data/dataloader/database-size/age/timeago/IOB/COB/treatment-curve/property/Loop/Profile/Settings/Sandbox/registry/realtime foundations and 25 v1 client/API files are adapted; 41 remain unresolved and two real-CGM bridges are fixed-scope exclusions. Nine locked server/data-plugin files also run unchanged as a separate 53/53-test gate. | Manually confirm route links. Update status only with whole-file upstream execution (`pass`) or complete named Workers-runtime contract coverage (`adapted`); keep generator/check green. |
...
Evidence candidate
`94b64e3e417806ddfa5243834ee8ce7eb9bc2f41` passes 577/577 tests in 50
Workers-runtime files plus 21/21 audit tests, one unchanged direct upstream
client test and 53/53 unchanged tests across nine locked server/data-plugin
files. It adds the complete named IOB, COB and treatment-to-curve contracts,
request-local formulas, bounded Treatment/Profile inputs, v2 property/Summary
dispatch and ddata marker placement. It retains the complete age/timeago,
...
Cloudflare version `1ed7fda2-c6bc-4137-9f53-25fcc16d8f40` (ordinal 59) is
100% active; deployment `dc4797d9-63ca-4039-904e-74b5808fd88f` was created
at `2026-07-20T05:56:10.151935Z` and reported a 31 ms startup. Wrangler
processed 248 unchanged official asset entries; the final dry run reported
1070.53 KiB raw / 196.22 KiB gzip, with only `ENTRY_STORE` and `ASSETS`.
This deployment had no explicit version annotation; none is invented.

The reusable credential-free remote smoke passed 63 assertions for health,
...
opt-in-disabled Loop/IOB/COB/CAGE/SAGE/IAGE, null disabled IOB/COB Summary
state, property-absent timeago and a parseable EIO4 polling open; missing-token
API3 Entries returned 401. Isolated tenant `public-smoke-1784527040684` reported
...
values, default-enabled `dbsize`, opt-in-disabled Loop/IOB/COB and an EIO4
polling open packet;
...
The deployed version adds IOB/COB and treatment-to-curve adapters while
...
contracts prove all 24 named IOB/COB/treatment-curve cases, Loop
...
A real Chromium session reloaded Cloudflare version 59, rendered the official homepage,
...
This is same-version evidence alongside version 59's remote API and Engine.IO
smoke.
