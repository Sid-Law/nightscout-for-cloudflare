...
`27ffb4256baeb4ecef0c91b91aa176cad2381504` passes 692/692 tests across 62
...
`70e49b47-bc2a-4f9e-82ba-b5e2eafbb922`; exact release
...
The candidate Wrangler dry-run reports 248 official assets, 1183.78 KiB raw /
218.53 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
...
exclusions. Version 74 passed remote API/EIO4 and the named real-browser gates;
the exact deployment evidence is recorded in `DEPLOYMENT.md`.
...
| Official page workflows | `views/**`, browser client/admin/report modules | **Partial.** Version 72 rendered the homepage/chart and live `0%` dbsize pill, Settings exposed the official language selector/About 15.0.7, and Admin Tools plus `clock-color` loaded without console errors. After a credentialed 25-entry simulator batch, the same official homepage rendered `101 mg/dL`, an upward trend and a populated chart. Version 73 rendered 26 official AR2 forecast dots. Version 74 reloaded the connected homepage, opened Settings with Admin authorized/About 15.0.7 and completed the unchanged Save workflow; the user-requested test-lab stale-data alarm checkboxes saved off. The only captured console rejections were browser autoplay-policy errors before interaction. Protected Profile/Food/Admin mutations, report generation and pushed live updates are not complete. | Add authenticated Profile Save/Food/profile-delete/admin/report workflows with console/network assertions in the final environment; keep real CGM/closed-loop traffic out of the simulated lab. |
...
Runtime candidate
`27ffb4256baeb4ecef0c91b91aa176cad2381504` passes 692/692 tests in 62
...
Cloudflare version `70e49b47-bc2a-4f9e-82ba-b5e2eafbb922` (ordinal 74) is 100%
active; deployment metadata activated at `2026-07-22T00:21:36.094Z`, and it
reported a 32 ms startup. Wrangler
processed 248 unchanged official asset entries; the final dry run reported
1183.78 KiB raw / 218.53 KiB gzip, with only `ENTRY_STORE` and `ASSETS`.
...
API3 Entries returned 401. Isolated tenant `public-smoke-1784679706217` reported
...
fresh tenant `public-smoke-1784679706217` and observed 262,144 SQLite bytes. It
...
A real browser session loaded Cloudflare version 74, rendered the connected
official homepage and opened Settings with Admin authorized/About 15.0.7. The
