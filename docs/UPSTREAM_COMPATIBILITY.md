...
`e1c380a29450a06621b15f7df7c904f81e1b1147` pass 276/276 tests across 27
...
addition to strict v1/v2 Status and all 16 locked API3 test-file contracts,
...
Cloudflare version
`f2801520-9c21-495a-acb2-337a157f82ec`; exact release evidence is recorded in
...
default test-file status is `unresolved`. At this audit point, 92 files are
`unresolved`, two real-CGM bridge files are `excluded-fixed-scope`,
all 16 API3 files plus `notifications-api.test.js` are `adapted`, and zero files are
...
| Upstream test tracking | `tests/**`, `upstream/contract-manifest.json`, `scripts/audit-upstream-contracts.mjs` | **Inventory complete; 17 adapted files.** All 111 files are tracked with a strict status/reason and heuristic candidate route associations: all 16 API3 files plus `notifications-api.test.js` are adapted by named Workers-runtime contracts, 92 remain unresolved and two real-CGM bridges are fixed-scope exclusions. | Manually confirm route links. Update status only with whole-file upstream execution (`pass`) or complete named Workers-runtime contract coverage (`adapted`); keep generator/check green. |
...
`e1c380a29450a06621b15f7df7c904f81e1b1147` pass 276/276 tests in 27
...
the locked v1/v2 Treatments `preBolus` fan-out. This increment completes all
16 locked API3 test-file contracts, adding create/update/patch/delete,
shape/AAPS and storage adapter/socket coverage to the prior six files.
Cloudflare version `f2801520-9c21-495a-acb2-337a157f82ec` was reported as the Current
Version by the direct Wrangler deployment, created at
`2026-07-19T18:34:44.097129Z`, which reported a 24 ms startup. Wrangler
