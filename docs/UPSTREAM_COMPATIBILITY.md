...
| Main Socket.IO namespace | `lib/server/websocket.js` | **Partial read-only EIO4 polling slice.** Exact `/socket.io` and `/socket.io/` requests route to tenant DOs. Persisted EIO4 sessions/queues, heartbeat, SIO5 root CONNECT, `clients`, read-only authorize/dataUpdate/ACK and loadRetro are tested across eviction and tenant boundaries. Polling and direct WebSocket status now use the same tenant-local DO profile/environment derivation as HTTP status. A SQL-derived DO alarm persists ping/pong/session/poll/POST deadlines and cleanup across eviction. The official page still loads the REST shim, so this is not a homepage realtime completion claim. | Switch the page only after safe tenant propagation and `/alarm`; add EIO3 HTTP if retained, root writes, persisted-change broadcasts and browser workflows. |
...
- HTTP, polling and direct-WebSocket status use one tenant-local DO source for
  profile units, configured thresholds, feature flags and reported auth delay;
  malformed/unknown profile units fall back without crossing tenant boundaries;
- `AUTH_FAIL_DELAY` is a documented platform safety difference: locked Node
  Nightscout accepts an unbounded numeric delay, while NSCF clamps the delay
  actually enforced and reported in every status transport to 0..60,000 ms
  (invalid values use the locked 5,000 ms default);
- websocket status keeps the locked field set/order; API/careportal enabled,
  boluscalc disabled, and no active profile remain platform assumptions;
