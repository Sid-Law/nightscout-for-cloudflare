...
| Main Socket.IO namespace | `lib/server/websocket.js`, `lib/data/{calcdelta,treatmenttocurve}.js` | **Partial server surface with EIO3/EIO4 polling, direct/upgrade WebSocket, official polling client, root writes, Profile Switch status and Treatment marker preprocessing adapted.** Exact `/socket.io` and `/socket.io/` requests route to tenant DOs. Persisted sessions/queues, protocol-aware heartbeat/framing, SIO4/SIO5 root CONNECT, `clients`, read/write/treatment-write authorize/ACK, initial/retro data and subsequent locked `dataUpdate` deltas are tested across polling, direct WebSocket, polling upgrade, reconstruction, mixed-protocol broadcast and tenant/authorization boundaries. Both protocols advertise `websocket`; exact probe/noop/upgrade, abort, cross-protocol, duplicate and alarm-timeout paths are tested through DO eviction. Initial and reconstructed delta snapshots run the locked Treatment-to-curve marker placement with profile units/raw-BG gating before the shared JSON budget reserves each item. Initial `status:true` authorization includes the latest one-year zero-duration Profile Switch as `status.activeProfile`; later switches compare against the persisted SQLite root baseline and publish fresh status after eviction. The byte-identical official Socket.IO 4.5.4 page client connects root and `/alarm` over polling by its own locked option. The write-shape contract covers all four events across six collections and ACK-before-delta delivery. A SQL-derived alarm persists transport deadlines. JSONP/binary and remaining non-Treatment plugin preprocessing remain missing; direct-WebSocket dequeue/send still has an at-most-once crash window. Version 93 adds protected Profile save/pushed-page evidence and release 94 adds EIO3 WSS evidence. | Complete remaining non-Treatment preprocessing and close the direct-send crash window; extend Mongo/BSON differential coverage without removing Free-plan bounds. |
...
Runtime candidate `a4e2267` passes 787/787 tests in 71
...
Cloudflare version `607733cc-2c05-44bc-bde3-17c94be68aff` is current as
project release 95 and reported a 28 ms startup. Wrangler 4.113.0 did not
print a numeric Cloudflare ordinal. It processed 250 Static Assets entries; the
final dry run reported 1291.33 KiB raw / 237.34 KiB gzip, with only
...
The reusable credential-free remote smoke passed 156 assertions for health,
...
metadata, the default-enabled `dbsize` and Basal properties,
...
`public-smoke-1784717472600` reported
...
- strict EIO3/EIO4 polling with `upgrades:["websocket"]`, direct WebSocket and
  each protocol's locked probe/noop/upgrade sequence;
