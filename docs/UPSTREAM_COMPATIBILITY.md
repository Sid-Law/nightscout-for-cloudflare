...
Deployed runtime candidate `31d0260` passes 763/763 tests across 69
...
`aa72dd6d-4307-41fc-8162-27211ed6dbb8`; exact release
...
The candidate Wrangler dry-run reports 250 Static Assets entries, 1269.08 KiB raw /
233.24 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
...
mutations now evaluate all fourteen producers in official server order on the
...
| Server plugins and calculations | `lib/plugins/index.js`, `lib/sandbox.js`, `lib/data/{dataloader,treatmenttocurve}.js`, `lib/profilefunctions.js` | **Static registry, complete named Sandbox/dataloader/dbsize/age/timeago/AR2/Basal/Treatment-Notify/Simple-Alarms/Error-Codes/OpenAPS/Pump/IOB/COB/BWP/treatment-curve surfaces, xDrip-js property/notification/pill source contract and fourteen automatic producers; broader background execution partial.** Pure ports of official `bgnow`, `direction`, `rawbg`, `upbat`, `ar2`, `basal`, `treatmentnotify`, `simplealarms`, `errorcodes`, `xdripjs`, `loop`, `openaps`, `pump`, `iob`, `cob`, `bwp`, `dbsize`, age/timeago, treatment-to-curve, shared runtime helpers and Profile calculations support v2 without rewriting formulas. BWP additionally maps its missing-data, unit, target, Temp Basal, notification and snooze paths and exposes Pebble fields. AR2 maps all ten named cases; Simple Alarms maps all five; Error Codes maps all eight unchanged upstream cases plus ten pure and three real-DO cases; xDrip-js has eleven pure/HTTP/property and three real-DO cases; Basal maps two and Treatment Notify six; OpenAPS/Pump map all 16 and IOB/COB/treatment-to-curve all 24 named cases plus DO/HTTP integration. The request-local Sandbox and static registry retain their complete contracts. Eleven complete client files run 42/42 unchanged, and twenty-one server/data-plugin files run 143/143 unchanged, against the locked upstream. AR2, Simple Alarms, Error Codes, Pump, OpenAPS, xDrip-js, Loop, BWP, CAGE, SAGE, IAGE, enabled Treatment Notify, opt-in Timeago and opt-in DBSize alerts share the persisted scheduler; unported algorithms are not fabricated. | Adapt the remaining plugin files through the Sandbox and deterministic tenant platform context, then add their automatic producers without inventing algorithms. |
| Notifications/admin state | `lib/notifications.js`, `lib/api/notifications-api.js`, `lib/adminnotifies.js`, `lib/server/pushnotify.js`, `lib/plugins/{maker,pushover,xdripjs}.js` | **Core notification processor, ACK/outlet persistence, fourteen automatic producers, Admin notices and provider protocol state adapted.** Schema v13 preserves last-emission state, v14 evaluates fourteen producers including default Error Codes plus opt-in xDrip-js and BWP, v15 persists Admin notices, v18 persists Pushnotify dedupe/receipts plus Maker All Clear state, and v20 persists xDrip-js's notification-throttle marker. All 15 locked Maker/Pushover/Pushnotify cases are represented, and the v1/v2 receipt callback consumes only a stored unexpired receipt. Activity/summary persistence, remaining plugin-bus integration and live external send/cancel remain missing. | Add the remaining alarm producers and activity/summary persistence; connect external delivery only after explicit destination authorization and a persisted outbox. |
...
Runtime candidate `31d0260` passes 763/763 tests in 69
...
Error Codes, Pump, OpenAPS, xDrip-js, Loop, BWP, CAGE, SAGE, IAGE, Treatment Notify, Timeago and opt-in
...
Cloudflare version `aa72dd6d-4307-41fc-8162-27211ed6dbb8` (ordinal 89) is
current and reported a 37 ms startup. Wrangler processed 250 Static Assets entries; the
final dry run reported 1269.08 KiB raw / 233.24 KiB gzip, with only
`ENTRY_STORE` and `ASSETS`.
This deployment had no explicit version annotation; none is invented.

The reusable credential-free remote smoke passed 125 assertions for health,
...
opt-in-disabled Loop/OpenAPS/Pump/xDrip-js/IOB/COB/CAGE/SAGE/IAGE, null disabled IOB/COB Summary
state, property-absent timeago, EIO3 polling and a real EIO4 WSS upgrade through
probe/noop/upgrade/root-CONNECT/`clients`;
default-enabled Error Codes status, the default-disabled xDrip-js property, missing-token
API3 Entries returned 401. Isolated tenant `public-smoke-1784702685161` reported
