...
`aa5029c40ba1d8a4f670c94bf1ee5ab48330a1d8` passes 658/658 tests across 59
Workers-runtime files plus 22/22 audit tests, 42/42 unchanged tests across
eleven complete upstream-client files and 91/91 unchanged tests across sixteen locked upstream
...
`80f36caa-6ec6-4eae-90e1-024b4675dcc0`; exact release
...
The candidate Wrangler dry-run reports 248 official assets, 1161.62 KiB raw /
214.68 KiB gzip and only `ENTRY_STORE` plus `ASSETS`. Post-deployment API and
...
default test-file status is `unresolved`. At this audit point, 18 files are
...
Runtime candidate
`aa5029c40ba1d8a4f670c94bf1ee5ab48330a1d8` passes 658/658 tests in 59
Workers-runtime files plus 22/22 audit tests, 42/42 unchanged direct upstream
client tests across eleven complete files and 91/91 unchanged tests across sixteen locked server/data-plugin
...
Cloudflare version `80f36caa-6ec6-4eae-90e1-024b4675dcc0` (ordinal 68) is 100%
active; its version was created at `2026-07-21T20:43:20.146Z`, deployment
metadata activated at `2026-07-21T20:43:20.982Z`, and it reported a 39 ms startup. Wrangler
processed 248 unchanged official asset entries; the final dry run reported
1161.62 KiB raw / 214.68 KiB gzip, with only `ENTRY_STORE` and `ASSETS`.
...
API3 Entries returned 401. Isolated tenant `public-smoke-1784666611302` reported
249,856 SQLite bytes,
...
fresh tenant `public-smoke-1784666611302` and observed 249,856 SQLite bytes. It
...
A real browser session loaded Cloudflare version 68, rendered the official
homepage, chart region, empty `---` value and live `0%` database-size pill.
The official Admin-notification link remained present. Settings exposed the
official language selector and About reported 15.0.7. Four fresh-tenant API
