...
| 12. Upstream regression suite | Tracked; 55 adapted files | Work through `docs/UPSTREAM_TEST_MANIFEST.md` in dependency order; all 16 API3 files, the named storage/concurrency/notification/data/property/Profile/realtime foundations and 25 v1 client/API files are adapted. The newest addition is the complete five-case Loop plugin file, after the 13-case concurrent-write and 24-assertion Profile files. 54 files remain unresolved and two are fixed-scope exclusions. |
...
both audit suites pass 20/20 and TypeScript passes. The manifest records 55
adapted, 54 unresolved and two fixed-scope excluded files. The official UI
