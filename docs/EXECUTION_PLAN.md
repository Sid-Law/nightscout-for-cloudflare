...
| 8. Engine.IO/Socket.IO | Partial protocol surface; EIO3/EIO4 XHR/JSONP polling, direct WebSocket and polling upgrade deployed | EIO3 and EIO4 XHR/JSONP polling, direct Hibernatable WebSocket and standard polling-to-WebSocket upgrade are routed to tenant `EntryStore` DOs with persisted protocol/session/queue authority. EIO4 preserves server-ping/client-pong and RS framing; EIO3 preserves client-ping/server-pong, length-prefixed framing and the two-stage SIO4 root CONNECT/`clients` order. Both follow the locked probe/noop/upgrade sequence and retain protocol authority across eviction. Schema v21 persists the initial JSONP callback index so later changed/omitted `j` values and URL-encoded `d=` POSTs retain locked framing after reconstruction. Root authorization, initial/retro data, deltas, official Treatment-to-curve preprocessing, four locked write events and API3 `/storage`/`/alarm` remain protocol-aware. The byte-identical official Socket.IO 4.5.4 browser client explicitly requests polling and uses root plus `/alarm`; standard and legacy external clients can use WebSocket. Release 99 keeps each direct-WebSocket FIFO prefix durable until every send succeeds and a post-send SQLite acknowledgement commits; a crash can replay but no longer silently loses the unsent durable copy. Binary packets remain the explicit non-ordinary transport gap. |
| 9. Real-time storage updates | Root server/client mutations, Treatment markers, Profile Switch status and API3 `/storage` implemented | Successful HTTP API3 mutations atomically enqueue official collection-room frames and root deltas; implemented v1/v2 changes publish root deltas in a follow-up DO transaction. Schema-v11 baseline and schema-v12 write authority survive reconstruction. Authorized client root writes preserve exact ACK/error ordering and queue any delta after the ACK; unauthorized/read-only sessions stay unable to mutate. Initial and reconstructed snapshots run the locked Treatment marker preprocessing before payload budgeting. Initial `status:true` authorization and later zero-duration Profile Switch writes publish the locked `activeProfile`, with the comparison marker surviving DO eviction. Version 93 observed a protected Profile mutation through the already-open official page's `dataUpdate`/`retroUpdate`. Locked upstream `dataUpdate` sends `dataWithRecentStatuses()`; server plugin evaluation is separate Sandbox work, not an additional root projection. Keep the unbounded `document_changes` journal and its future retention policy distinct from the bounded live transport queue. |
| 10. Alarms/background tasks | Generic SQLite scheduler + complete official notification producer task | Schema v14 stores logical tasks, due times, attempts and update times in SQLite. The DO's one Cloudflare alarm is derived from the minimum of persisted realtime, authorization-cleanup, task, schema-v16 debounce and optional schema-v17 lab-CGM deadlines. One `plugin-notifications` task evaluates all sixteen official producers in server order from a bounded SGV/MBG/DeviceStatus/Profile/Treatment/SQLite-stat context; Uploader Battery preserves future activation, heartbeat and exact 30-minute expiry, while BAGE preserves its threshold, 20-minute window, heartbeat and clear deadlines. Schema v20 additionally persists xDrip-js's small last-state throttle marker so eviction cannot reset its repeat interval. The disabled-by-default lab CGM uses the same alarm, writes one current SGV per five-minute due turn and never backfills an unbounded outage. Mutations run the leading edge; rapid batches receive exactly one persisted trailing evaluation after one quiet second or at the five-second max wait. Failures persist two-second exponential retry capped at five minutes; early at-least-once delivery is a no-op. Add summary/activity persistence and future maintenance/pruning. |
| 11. Server plugins/notifications | Complete property and notification producer registries + persisted core/Admin/push state | Stateless ports of every property-producing and notification-producing official server plugin plus shared `times`, `units`, `levels`, Profile calculations and the complete public `lib/sandbox.js` surface now exist. All sixteen producers are automatically evaluated by schema v14; schema v15 persists Admin notices, schema v16 persists data-update debounce and schema v20 persists xDrip-js state-notification cadence. Runtime State preserves the normal request-time `loaded` value. BAGE preserves latest nonfuture Pump Battery Change selection, day/hour display, Summary age, threshold metadata and automatic clear. Uploader Battery preserves `UPBAT_*`, recent-30-minute selection, per-device ten-minute minimum, multi-device message, future activation, heartbeat and clear. Version 81 completely maps the locked Maker, Pushover and Pushnotify files: schema v18 persists dedupe leases, receipts and Maker All Clear state, and v1/v2 expose the official receipt callback. The processor preserves priority, information/announcement handling, snooze arbitration and automatic all-clear. External Pushover/IFTTT send/cancel remains disabled until explicitly authorized and connected through a persisted outbox. |
...
The deployed runtime candidate `56353da` retains schema-v15 persisted Admin
...
`plugin-notifications` task to the locked Uploader Battery, AR2, Simple Alarms, Error Codes, Pump, OpenAPS,
xDrip-js, Loop, BWP, CAGE, SAGE, IAGE, BAGE, Treatment Notify, Timeago, DBSize and notification-
processor modules. Mutations evaluate all sixteen in official server order from
...
authorization and notification-ACK contracts remain green. The v2-only Loop
...
`339263b5-c3d5-400a-b3b3-0c6299722d32` is active as project release and
Cloudflare ordinal 100. It reported a 36 ms startup and processes 250 Static
Assets entries. The Wrangler 4.113.0 dry run reports 1319.52 KiB raw / 242.79
KiB gzip and exposes
only `ENTRY_STORE` plus `ASSETS`. The 73-file Workers-runtime suite passes
799/799, all four audit suites pass
...
Version 100 adds durable Engine.IO JSONP polling for both protocol generations.
The locked Engine.IO 6.2.1 oracle fixes the callback index at the initial
handshake; schema v21 stores it with the SID, and later XHR-style polls or
URL-encoded form POSTs preserve callback/content-type/payload framing across
Durable Object reconstruction. Five focused JSONP cases and a v20-to-v21 live
session migration case raise the complete runtime gate to 73 files / 799
tests. The 213-assertion remote smoke passed on
`public-smoke-1784727265264`; the unchanged homepage rendered `118 mg/dL`, a
trend arrow and two SVGs without a dialog or console error, and Admin loaded
seven default roles as `Admin authorized`.

Version 99 removes the direct-WebSocket dequeue-before-send loss window. The
bounded SQLite FIFO is now peeked without mutation; only after every selected
frame has been synchronously handed to `WebSocket.send()` is the exact prefix
validated and deleted in one transaction. A crash at the final boundary can
repeat a frame because network send and SQLite cannot be one Cloudflare
transaction, but it cannot silently discard an unsent durable frame. The
25-case repository/state file includes reconstruction and mismatched-ACK
rollback evidence; the complete runtime gate is 72 files / 793 tests. The
177-assertion remote smoke passed on `public-smoke-1784725389106`; the official
homepage remained at `/`, rendered two SVGs and advanced from `125 +4 ↗` to
`128 +4 ↗` while open without a dialog.

Version 98 fixes the unchanged official Admin Tools cleanup display without
changing the standard collection-delete contract. Locked v15.0.7 server code
returns MongoDB 5's `{acknowledged,deletedCount}`, but the unchanged Entries
and Treatments Admin plugins still read legacy `retVal.n`. The Worker adds
`n`/`ok` aliases only when the request is same-origin and its Referer path is
exactly `/admin/`; direct API callers keep the exact modern result. Two new
server-workflow contracts and the related Entries/Treatments/collections
regressions pass. A fresh EIO3 replay and the complete 177-assertion remote
smoke passed after one non-repeating post-cutover ping failure. The real Admin
buttons rendered `0 records deleted` for both collections and stayed
authorized; the homepage displayed current simulated glucose and two charts.
...
snapshot. Ddata now retains the full latest Profile store, the full bounded
one-day DeviceStatus loader, raw Food without a fabricated `mills`, the exact
ordinary/Profile-Switch/age-event Treatment loaders, `lastProfileFromSwitch`,
Activity and all eight derived Treatment arrays. Root authorization still
filters `@@@@@` Profile history, keeps ten recent statuses per device/type and
omits Activity/derived buckets. A bounded 15-minute `updated_at` window models
the upstream in-memory hot cache so a newly submitted backdated API3 Treatment
is immediately visible after DO eviction; explicit frames stay historical. A
165-assertion remote smoke passed on `public-smoke-1784718918966`. Browser
acceptance showed `129 mg/dL`, two-minute-old simulated data, the official SVG
chart with no console error, and an authorized Admin Tools page with all seven
system roles.

Version 95 completes the locked Activity portion of API v2 ddata without
changing the main Socket.IO authorization payload. Current ddata exposes the
upstream two-day, oldest-first four-field Activity projection and de-duplicates
equal instants; explicit historical frames additionally cap rows at the frame
time and return `page.frame/after`. The root snapshot remains based on
`dataWithRecentStatuses()` and therefore correctly omits Activity. A
156-assertion remote smoke passed on `public-smoke-1784717472600` after
Cloudflare propagation, retaining real EIO3 direct WSS and both EIO3/EIO4
polling upgrades. Browser acceptance showed `117 mg/dL`, trend/chart data,
zero dialogs/errors and an authorized Admin Tools page. Derived summary/plugin
Activity persistence remains a separate background-work gap.

Version 94 completes the locked `allowEIO3` WebSocket transport for ordinary
legacy clients. Direct EIO3 WebSocket and polling-to-WebSocket upgrade now use
the same Hibernatable Durable Object path as EIO4 while retaining EIO3 length-
prefixed payloads, client-ping/server-pong and SIO4 packet forms. The upstream
Socket.IO 4.5.4 server supplied exact direct-open and probe/noop/upgrade wire
oracles. A 150-assertion remote smoke passed on
`public-smoke-1784713764749`, including real EIO3 direct WSS and both EIO3/EIO4
polling upgrades. Browser acceptance showed current `113 mg/dL`, one-minute-
old simulated data and the official chart; Admin Tools loaded seven roles and
`Admin authorized`. JSONP/binary remained; release 99 later closed the
direct-send dequeue-before-send loss boundary and release 100 closed JSONP
polling. Binary packets remain.

Version 93 prepares clean-source deployment without changing the runtime
...
The immediately preceding authenticated Profile rename/save was observed in
the open official homepage as `dataUpdate` and `retroUpdate`; the original
Profile name was restored and reloaded.

Version 92 restores `POST /api/v2/notifications/loop` without inventing a v1
alias. Nine Workers contracts cover exact configuration/Profile error order,
Temporary Override/cancel, Remote Carbs/Bolus, ES256 provider JWT verification,
development/production APNs requests, bounded error handling, network failure,
form parsing and permission enforcement. The current lab has no `LOOP_APNS_*`
credentials and no real remote instruction was sent. The 139-assertion remote
smoke passed on `public-smoke-1784710160605`, proved the active route reaches
HTTP 401 rather than the prior 404, retained EIO4 upgrade and EIO3 polling, and
reported 307,200 SQLite bytes. A fresh official homepage displayed `117
mg/dL`, `+3`, one-minute-old data and its chart; the official Admin page loaded
all cleanup groups, seven built-in roles, zero subjects and `Admin authorized`.

Version 91 completes every locked official server `checkNotifications`
producer. Uploader Battery maps the three `UPBAT_*` settings into HTTP/Socket
extended settings and request-time property thresholds, then evaluates bounded
DeviceStatus in the locked registry position before AR2. Source/HTTP/property
tests cover the exact multi-device/voltage message and opt-in gate; a real
SQLite DO case covers future activation, heartbeat repetition, exact
30-minute-plus-one-millisecond expiry, automatic All Clear and live `/alarm`.
The 134-assertion remote smoke passed on `public-smoke-1784706560346`, including
default UPBAT configuration/property output, EIO4 upgrade and EIO3 polling. A
fresh official page displayed `113 mg/dL`, `-1`, one-minute-old data, the chart
and About 15.0.7 with zero captured browser warnings/errors.

Version 90 completes every `setProperties` producer in the locked official
server registry. Default-enabled Runtime State returns `loaded`; opt-in BAGE
ports the official source selection, day/hour display, notes, Summary age,
`BAGE_*` settings and information/warning/urgent notification request. BAGE is
the fifteenth schema-v14 producer, with future threshold activation, inclusive
minute-20 alerting, minute-21 clear, heartbeat repeat and live `/alarm`
publication proven by a real SQLite DO case. The 129-assertion remote smoke
passed on `public-smoke-1784704705985`, including default Runtime State,
default-disabled BAGE, EIO4 upgrade and EIO3 polling. A fresh official page
displayed `127 mg/dL`, the chart and About 15.0.7 with zero captured browser
warnings/errors. Uploader Battery remains the one official server notification
producer not yet connected to the automatic scheduler.
...
snapshot RPC. Enabled IOB/COB/BWP/BAGE Summary state is deployed and every
official server property producer is request-time adapted; summary/activity
persistence still requires Milestone E.
...
- a tenant-local API3 `/alarm` namespace with independent connection, native
  ...
  feeds all sixteen official producers through that outlet in locked order,
...
Engine.IO binary packets, live external provider delivery,
summary/activity persistence and nine upstream test files (seven unresolved
...
EIO3 and EIO4 polling upgrades are complete and independently public-tested.
Version 93 closes the named protected
Profile-save push gate; additional plugin workflows remain incremental gates.

The deployed XHR/JSONP polling slice is intentionally bounded to 256 sessions per tenant,
...
Release 99 removes pre-send loss by retaining direct-WebSocket frames until
post-send acknowledgement; a crash may replay at that non-atomic boundary.
Release 100 persists JSONP mode/callback authority with the SID. The remaining
transport slice is binary packet support; root plugin properties are not added
because locked upstream `dataUpdate` does not include them.
...
2. **Complete for official server `setProperties` producers:** the ddata helper,
   aggregate route, property picker/pretty mode and all locked server property
   plugins now run through the request-local registry, including Runtime State
   and BAGE. Finish summary/activity persistence, the v2-only notification loop
   and remaining authorization surfaces; inherited v1/v2 notification ACK is
...
2. **Partial:** EIO4/SIO5 and EIO3/SIO4 polling sessions, direct Hibernatable
   WebSocket and polling-to-WebSocket upgrade,
   ...
   sequence. Both protocols advertise and complete the locked WebSocket
   upgrade; XHR and JSONP polling are complete, while binary packets remain
   rejected.
3. **Complete for EIO3 and EIO4:** direct WebSocket open and polling-to-
   WebSocket upgrade are implemented and tested across DO eviction, candidate
   abort, duplicate admission and alarm-driven timeout.
4. **Complete for the named `/storage` and `/alarm` transport slices:** preserve
   ...
   live-only delivery and tenant isolation. All sixteen official automatic
   producers now publish through the outlet in server order, including
   Uploader Battery, Error Codes, xDrip-js, BWP and BAGE.
5. **Complete for the locked root write-shape slice:** `dbAdd`, `dbUpdate`,
   ...
   broadcast order. Profile Switch status and Treatment marker preprocessing
   are persisted/recomputed; locked upstream root projection is preserved.
   Extend broader Mongo/BSON behavior separately.
...
7. **Complete for the official polling client switch:** the locked Socket.IO
   ...
   gates pass. The protected Profile save/push gate also passes. Keep both
   protocol-generation WebSocket gates green; the locked page remains
   polling-only by upstream choice.

### Milestone E — background/server behavior

1. **Generic substrate and official producer coverage complete:** schema v14
   persists the task table and shares one derived alarm with realtime/auth
   deadlines. All sixteen official notification producers share one automatic
   task; add remaining non-plugin jobs.
...
4. Retain the adapted dataloader/Sandbox/database-size path and complete
   remaining non-plugin server behavior through a persisted tenant context.
5. **Complete for all sixteen official producers:** the engine consults persisted `/alarm`
   ...
   Keep this path green without inventing medical algorithms.
...
The clean-source `build` and `deploy` scripts plus sole `API_SECRET` binding
are audited for Deploy to Cloudflare. Publishing a public GitHub/GitLab source
URL and completing a fresh Cloudflare-account button run are release gates,
not runtime-port tasks.
